(() => {
  ...
  const FRENCH_OPTIONS = ["NA","3 Fr","4 Fr","4.5 Fr","5 Fr","6 Fr","7 Fr","8 Fr","9 Fr","10 Fr","12 Fr","14 Fr","16 Fr","18 Fr","20 Fr","22 Fr","24 Fr"];
  const ITS_DEVICE_TYPES = ["CVPC", "CVC", "PICC", "CATT HD", "C. PUERTO", "ONFALOCLISIS"];
  const ITU_MATERIAL_TYPES = ["NA","SILICON","LATEX"];
  const ITU_DEVICE_STATES = ["NA","A DERIVACION","CIRCUITO CERRADO"];
  ...
  const NAVM_ORAL_HYGIENE_TYPES = ["NA","CLORHEXIDINA","SALINA","CEPILLO DENTAL"];
  ...
  function renderPackageSpecificFields(date, patientId, device, index) {
    ...
    if (type === "ITS - CC") {
      return h("div", { class: "package-fields" }, [
        ...
        device.deviceType === "CATT HD" ? renderButtonGroup("Tipo CATT HD", ["NA", "PERMACATH", "MAHURKAR"], device.deviceSubtype, value => update({ deviceSubtype: value })) : "",
    ...
    return h("div", { class: "iaas-page follow-page expediente-page" }, [
      ...
      h("section", { class: "iaas-grid two" }, [
        h("article", { class: "iaas-panel" }, [
          h("h2", {}, ["Historial diario de invasivos"]),
  ...
  async function saveRoundEntry(date, patientId, requestedStatus, goNext) {
    ...
    (draft.deviceDrafts || []).forEach((device, index) => {
      device.draftId ||= preventiveDraftId();
      device.reviewDate = date;
      device.packageReviewId ||= device.savedEpisodeId || device.episodeId || (patientId + "|" + date + "|" + device.draftId);
      ...
      const episode = {
        episodeId: device.savedEpisodeId || device.episodeId || buildDeviceEpisodeId(patientId, deviceDisplayName(device), device.installationDate, (device.french || "") + "|" + (device.draftId || index)),
        patientId,
    ...
    const entry = {
      ...
      packageReviews: mergePackageReviewList(previousEntry.packageReviews || [], packageReviews),
    ...
    saveStore();
    const syncItem = await enqueueWrite({ type: "roundEntry", date, patientId, entry, patient, episodes: unique(createdEpisodeIds).map(id => store.deviceEpisodes[id]).filter(Boolean) });
  ...
  function setAllPreventiveChecks(checks, value) {
  ...
  function sanitizePreventiveRoundText(value) {
    const text = cleanCell(value)
      .replace(/\s*(?:\/|\||;)+\s*/g, " / ")
      .replace(/(?:^|\s)\/+(?=\s|$)/g, " ")
      .replace(/\s+/g, " ")
