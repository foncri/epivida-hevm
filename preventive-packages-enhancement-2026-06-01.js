  ...
  function renderPreventiveActionsPanel(date, patientId, patient, draft) {
    ...
    return h("section", { class: "iaas-panel preventive-bottom-panel" }, [
      ...
      h("div", { class: "preventive-bottom-grid" }, [
        h("article", { class: "preventive-action-card" }, [h("h3", {}, ["Cambio de cama o servicio"]), h("label", { class: "field" }, [h("span", {}, ["Nuevo servicio"]), h("select", { value: normalizeService(movement.service || patient.currentService || ""), onchange: event => patchMovement({ service: event.target.value }) }, preventiveMovementServiceOptions(patient, movement).map(service => h("option", { value: service }, [service]))) ]), h("label", { class: "field" }, [h("span", {}, ["Nueva cama"]), h("input", { value: movement.bed ?? patient.currentBed ?? "", oninput: event => patchMovement({ bed: event.target.value }) })])]),
