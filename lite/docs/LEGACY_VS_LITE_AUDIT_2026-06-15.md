...
| Monitoreo | `renderEpidemiologicalMonitoringPage`, filtros, etiquetas IAAS/riesgo/vig, paneles de pendientes y alertas. | `monitorService`, `modules/monitoreo`, filtros locales, paginacion, metricas, `operationalAlertService` para avisos preventivos/IAAS/cultivos/OPD/sync y `snapshotService.snapshotTrend()` para tendencia corta desde `daily_snapshots` sin store global. | Migrado ampliado; falta QA longitudinal con snapshots reales anonimizados. |
...
| Reportes | Reportes/print/Sheets y `computeRangeStats(30)` en runtime. | `reportService`, `exportService`, CSV/JSON protegido, snapshots por rango, `snapshotService` para tendencia de Inicio, cedulas preventivas, historicos por cursor/bloque y `exports_log`. | Migrado base ampliado; Excel opcional pendiente de decision y snapshots mensual/anual pendientes. |
