...
| Cloudflare | `https://epivida-hevm.pages.dev/epivida-lite-build.json` debe responder `release: 2026-06-16-parity08`, `Cache-Control: no-cache`, despues de publicar este bloque. |
...
| Importacion | `parseImportInput`, `buildImportDraft`, `buildImportPlanV2`, `import-census-repair.js` y parsers de hotfix. | `importService`, `censusRepairService`, `reconciliationService`, preview, protegidos, `census_days`, snapshots y auditoria; el reparador detecta censos hospitalarios legacy con encabezados rotos, filas guia, fechas Excel, RFC/afiliacion, sector, edad en meses/dias, estados intubados, diagnosticos y observaciones. | Migrado ampliado; falta Excel dinamico opcional y mas fixtures anonimizados. |
...
| Cloudflare build marker | Esperado tras publicacion: `2026-06-16-parity08`, `no-cache`. |
