...
| Ronda | Ronda por fecha, mapa de camas, paciente individual, guardar y siguiente. | `modules/ronda-paquetes` implementa flujo completo con cola offline; `bedBoard.js`, `preventiveForms.js`, `saveRoundFlow.js`, `roundHelpers.js` y `roundPatientUtils.js` separan mapa, formularios, guardado/drafts y logica pura. | Archivo principal aun concentra la vista de paciente individual e historial preventivo. | P0 | Continuar split en `roundPage` y `patientRound`. | `npm run validate:round`; entrar a monitoreo no carga ronda. |
...
- Continuar split de `ronda-paquetes/index.js` con paciente individual e historial preventivo; formularios ya viven en `preventiveForms.js`.
