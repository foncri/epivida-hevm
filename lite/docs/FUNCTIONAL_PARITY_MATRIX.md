...
| IAAS | Sospecha, confirmacion, tipo, estado, cierre y relacion con dispositivos. | `iaasService`, `modules/epi-iaas`; el formulario captura criterios, origen, vitales, labs, plan, cultivo y antimicrobiano inicial; expediente lee IAAS por paciente sin cargar todo el hospital. | Falta UI avanzada por secciones y reglas/cedulas clinicas versionadas. | P0 | Catalogar criterios heredados y dividir seguimiento por secciones lazy. | Crear/cerrar IAAS, guardar criterios/vitales/labs/plan y verificar reglas por rol. |
...
| Expediente | Vista por paciente con censos, rondas, dispositivos, IAAS y auditoria. | `modules/expediente` usa `expedienteService`; lee paciente por ID desde activo/archivo, mezcla dispositivos activos, primera pagina de rondas, `devices_archive`, IAAS, cultivos, antimicrobianos y auditoria por paciente. | Falta UI de siguiente pagina/cursor visible para historicos largos. | P0 | Ampliar `expedienteService` con cursores por seccion. | Abrir expediente no carga historicos globales ni archivos sin filtro. |
...
- Paginacion visible por seccion en expediente para continuar rondas, IAAS e historicos largos mas alla de la primera pagina.
- IAAS con reglas/cedulas clinicas versionadas y UI avanzada de seguimiento por secciones.
