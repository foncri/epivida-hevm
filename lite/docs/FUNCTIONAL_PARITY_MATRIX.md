...
| Ronda | Ronda por fecha, mapa de camas, paciente individual, guardar y siguiente. | `modules/ronda-paquetes` implementa flujo completo con cola offline; `bedBoard.js`, `saveRoundFlow.js`, `roundHelpers.js` y `roundPatientUtils.js` separan mapa, guardado/drafts y logica pura. | Archivo principal aun concentra paciente individual y formularios preventivos. | P0 | Continuar split en `roundPage`, `patientRound` y `preventiveForms`. | `npm run validate:round`; entrar a monitoreo no carga ronda. |
...
- Continuar split de `ronda-paquetes/index.js` con paciente individual y formularios.
