...
| Ronda | Ronda por fecha, mapa de camas, paciente individual, guardar y siguiente. | `modules/ronda-paquetes` implementa flujo completo con cola offline; `bedBoard.js`, `roundHelpers.js` y `roundPatientUtils.js` ya separan parte del mapa/logica pura. | Archivo principal aun concentra paciente individual, formularios preventivos y guardado. | P0 | Continuar split en `roundPage`, `patientRound`, `preventiveForms`, `saveRoundFlow`. | `npm run validate:round`; entrar a monitoreo no carga ronda. |
...
- Continuar split de `ronda-paquetes/index.js` con paciente individual, formularios y guardado.
