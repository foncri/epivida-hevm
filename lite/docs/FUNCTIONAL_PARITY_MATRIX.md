...
| Dispositivos | Episodios, instalacion, retiro, sitio, French, curacion, infeccion. | `deviceService`, `modules/dispositivos`, uso en ronda; el retiro marca `devices_active` inactivo y escribe copia en `devices_archive`. | Falta expediente historico paginado leyendo `devices_archive` por paciente. | P0 | Crear `expedienteService` paginado para activos/archivo y auditoria relacionada. | Retiro crea `devices_archive/{episodeId}`, deja activo fuera de listados activos y visible en expediente. |
...
- Expediente historico paginado leyendo `devices_archive`.
