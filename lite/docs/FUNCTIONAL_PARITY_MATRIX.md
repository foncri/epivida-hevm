...
| IAAS | Sospecha, confirmacion, tipo, estado, cierre y relacion con dispositivos. | `iaasService`, `modules/epi-iaas`; el formulario puede adjuntar cultivo y antimicrobiano inicial sin cargar historicos. | Falta seguimiento por secciones y criterios heredados. | P0 | Expandir `iaasService` con criterios, origen, followUp y relacion device. | Crear/cerrar IAAS y verificar reglas por rol. |
